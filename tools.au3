func relax($notes,$spinners)
   GUICtrlSetData($Labelstatus2,"sit back and relax!")
   $i = 1
   $exit = 0
   $k = 1
...
func aimbot($notes,$spinners)
   GUICtrlSetData($Labelstatus2,"aimbottin' it out, dude!")
   $i = 1
   $j = 2
   $k = 1
   ;$speed = $bpm[1][2]
   ...
   while 1
	  ...
	  if $ms >= $notes[$i][3][1] Then
		 ...
		 if $notes[$i][0][1] = "spinner" Then
			mousemove($notes[$i][1][1],$notes[$i][2][1],0)
		    spin($spinners[$k][4])
			mousemove($notes[$i][1][1],$notes[$i][2][1],0); Then if ($notes[$3]) end
			$k += 1
		 EndIf
		 EndIf
		 while 1
