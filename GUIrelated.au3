func optionsgui();create the GUI of options
   ...
   $Label5 = GUICtrlCreateLabel('timing correction - (relax will tap x milliseconds before, "x" is random between min and max, change this to improve undetectability', 32, 32, 611, 17)
   ...
   $Label8 = GUICtrlCreateLabel('hold key/mouse button - (relax will hold button for x milliseconds before releasing it, "x" is random between min and max, change this to improve undetectability)', 32, 112, 732, 17)
   ...
   $Label11 = GUICtrlCreateLabel("singletap interval - (the interval between single taps, if the time between a note and another is lower than this value then relax will tab the secondary button)", 32, 192, 755, 17)
   $maxintervalin = GUICtrlCreateInput($maxinterval, 24, 216, 65, 21)
   $Label12 = GUICtrlCreateLabel("keys - (the primary and secondary keyboard buttons set in your game options, reverse it if you tap then reversed and check `mouse buttons` to use mouse clicks)", 32, 272, 766, 17)
   ...
   EndIf
   $Label20 = GUICtrlCreateLabel("all values in milliseconds - (0 or negative in max value is not recommended but some fast songs may require lower values)", 152, 59, 571, 17)
   $Label21 = GUICtrlCreateLabel("all values in milliseconds - (0 is not recommended but some fast songs may require lower values)", 152, 139, 453, 17)
   $Label22 = GUICtrlCreateLabel("milliseconds - (for single tap only set this value to 0, which i don't recommend)", 88, 219, 242, 17)
   ...
   $Label17 = GUICtrlCreateLabel("time to move mouse - (x milliseconds before the note appears, it will start moving the mouse)", 32, 32, 427, 17)
   $movetimein = GUICtrlCreateInput($movetime, 24, 56, 89, 21)
   $Label18 = GUICtrlCreateLabel("milliseconds", 112, 59, 59, 17)
   ...
   $TabSheet5 = GUICtrlCreateTabItem("spinhack")
   ...
   $Label32 = guictrlcreateLabel("scan precision - (the lower the value, the slower it is but less likely to get error 12)",32,32,333,17)
   $scanprecisionin = guictrlcreateinput($scanprecision,24,59,73,21)
   $Label33 = GUICtrlCreateLabel("slider length precision - (the lower the value, the slower it is but gives more precise slider timing, decrease it if you don't care about load time (or increase if you have a old processor))",32,112,700,17)
   $bezierprecisionin = guictrlcreateinput($bezierprecision,24,139,73,21)
   GUICtrlCreateTabItem("")
   $okbutton = GUICtrlCreateButton("ok then!", 0, 336, 75, 25)
...
func hotkeysgui(); create the GUI of hotkeys
   #Region ### START Koda GUI section ### Form=
   $Form3 = GUICreate("hotkeys", 193, 177, 276, 191)
   ...
   $okbutton = GUICtrlCreateButton(":-)", 8, 144, 75, 25)
   $cancelbutton = GUICtrlCreateButton(":-(", 112, 144, 75, 25)
   GUISetState(@SW_SHOW)
   #EndRegion ### END Koda GUI section ###
...
func mainguiloop(); main gui loop for data
   ...
   Select
	  ...
	  case $nMsg = $initbutton
		 ...
		 GUICtrlSetData($Labelstatus2,"let's kick it! waiting...")
...
func buttonpressed($hWnd, $Msg, $wParam, $lParam); detects if stop button is pressed
   if guictrlread($Labelstatus2) = "waiting for you, squire." then
